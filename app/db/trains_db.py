...
def get_db():
    ...
    uri = os.getenv("MONGODB_TRAINS_URI", os.getenv("MONGODB_URI", "")).strip()
    if not uri:
        return None
    try:
...
async def _seed_static_data(db):
    ...
    if count == 0:
        ...
        logger.info(f"Trains DB: seeded {len(stations)} stations")
    else:
        logger.info(f"Trains DB: stations already seeded ({count})")
    ...
    if count == 0:
        ...
        logger.info(f"Trains DB: seeded {len(trains)} trains")
    else:
        logger.info(f"Trains DB: trains already seeded ({count})")
